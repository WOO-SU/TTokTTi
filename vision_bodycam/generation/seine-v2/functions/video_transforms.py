...
class WebVideo256256:
    ...
    def __call__(self, clip):
        ...
        # print('before resize', clip.shape)
        if h < 256:
            clip = resize_scale_with_height(clip=clip, target_size=258, interpolation_mode=self.interpolation_mode)
            # print('after h resize', clip.shape)
        if w < 256:
            clip = resize_scale_with_weight(clip=clip, target_size=258, interpolation_mode=self.interpolation_mode)
            # print('after w resize', clip.shape)
        clip_center_crop = center_crop(clip, self.size)
        # print(clip_center_crop.shape)
        return clip_center_crop
...
class InternVideo320512:
    ...
    def __call__(self, clip):
        ...
        Returns:
            torch.tensor: scale resized / center cropped video clip.
                size is (T, C, crop_size, crop_size) 1.6
        """
        # add aditional one pixel for avoiding error in center crop 
        h, w = clip.size(-2), clip.size(-1)
        if h < w and h * 1.6 < w: 
            clip_center_crop = center_crop(clip, (h, h * 1.6))
            clip = resize(clip_center_crop, (320, 512))
            return clip
        else:
            # print('before resize', clip.shape)
            if h < 320:
