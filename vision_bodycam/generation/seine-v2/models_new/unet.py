...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    @classmethod
    def from_pretrained_2d(cls, pretrained_model_path, subfolder=None, use_concat=False, finetuned_image_sd_path=None):
        logger.info(f'Loading stable diffusion from {pretrained_model_path}')
        ...
        #### check, whether this is able to load finetuned sd ###
        if finetuned_image_sd_path is not None:
            logger.info(f'==> Replacing the pre-trained sd model {model_file} with finetuned sd model {finetuned_image_sd_path}')
            temp_state_dict = torch.load(finetuned_image_sd_path, map_location="cpu")['state_dict']
            state_dict = {}
        ...
        if use_concat:
            ...
            logger.info('Loading weights from SD')
        else:
            for k, v in model.state_dict().items():
                if '_temp' in k:
    ...
    #     #### check, whether this is able to load finetuned sd v1.4 weights
    #     # temp_state_dict = torch.load('/mnt/petrelfs/xujilan/checkpoints/animatediff/checkpoint.ckpt', map_location="cpu")['state_dict']
    #     # state_dict = {}
    #     # for k, v in temp_state_dict.items():
    #     #     newk = k.replace('module.','')
    #     #     state_dict[newk] = v
    ...
    #         # breakpoint()
    #         res = model.load_state_dict(new_state_dict)
    #         print('Loading weights from SD', res)
    #     else:
    #         for k, v in model.state_dict().items():
    #             # print(k)
    #             if '_temp' in k:
...
if __name__ == '__main__':
    ...
    unet = UNet3DConditionModel.from_pretrained_2d(pretrained_model_path, subfolder="unet", use_concat=False, num_class_embeds=None).to(device)
